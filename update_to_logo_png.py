...
for root, dirs, files in os.walk('.'):
    for f in files:
        if f.endswith('.html'):
            ...
            # replace any variations
            content = re.sub(r'assets/image copy\.png(\?v=\d+)?', 'assets/logo.png?v=8', content)
            content = re.sub(r'assets/image\.png(\?v=\d+)?', 'assets/logo.png?v=8', content)
